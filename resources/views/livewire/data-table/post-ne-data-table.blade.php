<div>
    ...
    <div class="d-sm-flex justify-content-between align-items-center flex-warp">
        <div class="row-action mb-2 mb-sm-0 fs-6">
            ...
            <span class="publish">
                |
                <a href="{{ route('backend.post_ne', ['status' => 'publish']) }}" class="">
            ...
            <span class="draft">
                |
                <a href="{{ route('backend.post_ne', ['status' => 'draft']) }}" class="">
            ...
            <span class="delete">
                |
                <a href="{{ route('backend.post_ne', ['status' => 'trash']) }}" class="">
    ...
    <div class="table-attrs my-2 d-flex justify-content-between flex-wrap gap-2">
        ...
        <div class="d-none">
            <button data-bs-toggle="collapse" href="#multiCollapseExample1" aria-expanded="false"
                aria-controls="multiCollapseExample1" type="button" class="btn btn-light"><i
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-striped dt-responsive nowrap w-100 border">
            ...
            <tbody>
                @forelse ($posts as $p)
                <tr wire:key="{{ $p->id }}">
                    {{-- <td class="text-nowrap text-center" style="width: 70px;">
                            {{ ($posts->currentPage() - 1) * $posts->perPage() + $loop->iteration }}
                    </td> --}}
                    <td class="text-nowrap">
                        <span class="fw-600">{{ $p->post_title }}</span>
                        {{ $p->post_status != 'publish' ? ' — Draft' : '' }}
                        <br />
                        <div class="row-action fw-700">
                            @if ($status != 'trash')
                            <span class="edit">
                                <a href="{{ route('backend.post_ne.edit', $p->id) }}"
                            ...
                            <span class="restore">
                                <a href="javascript:void(0);"
                                    wire:confirm="Are you sure you want to restore this post?"
